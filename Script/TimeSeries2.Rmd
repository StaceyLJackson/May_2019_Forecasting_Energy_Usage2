...
install.packages(c("RMySQL","lubridate","ggplot","padr","zoo","rstudioapi","forecast"))

pacman::p_load(RMySQL, dplyr, lubridate, ggplot2,ggfortify,plotly, forecast,zoo, padr,rstudioapi)
...
#create empty rows
newDF2 <- pad(newDF2,by="DateTime",break_above=2)
...
ts_month<-ts(house_month$month_total, frequency=12,start(2007,1))

autoplot(ts_month)


## Decompose into trend, seasonal and remainder
decomposed_ts_month <- decompose(ts_month)

autoplot(decomposed_ts_month)


## Check variance??????
##variance_ts_month<-apply(decomposed_ts_month,2,var)
                         

##create training and test sets

train_month<-window (ts_month,start(2007,1), end=c(2008,12))

test_month <- window(ts_month,start(2009,1))
...
##make forecasts
forecast_HW_month <- forecast:::forecast.HoltWinters(HW_month,h=12)
...
##make forecasts
forecast_AR_month<-forecast:::forecast.Arima(AR_month,h=12)
